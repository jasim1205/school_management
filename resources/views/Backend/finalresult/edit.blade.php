    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="bs-stepper-content">
                <form class="form needs-validation" method="post" enctype="multipart/form-data" action="{{route('finalresult.update',encryptor('encrypt',$finalresult->id))}}">
                    @csrf
                    @method('Patch')
                    <div id="test-nl-1" role="tabpanel" class="bs-stepper-pane" aria-labelledby="stepper2trigger1">
                        <div class="row g-3">
                            <div class="col-12 col-lg-6">
                                ...
                                @forelse($exam as $e)
                                    <option value="{{$e->id}}" {{ old('exam_id',$finalresult->exam_id)==$e->id?"selected":""}}> {{ $e->exam_name}}</option>
                                @empty
                            ...
                            <div class="col-12 col-lg-6">
                                ...
                                @forelse($student as $s)
                                    <option value="{{$s->id}}" {{ old('student_id',$finalresult->student_id)==$s->id?"selected":""}}> {{ $s->first_name_en}}  {{ $s->last_name_en}}</option>
                                @empty
                                    <option value="">No Role found</option>
                                @endforelse
                                </select>
                            ...
                            <div class="col-12 col-lg-6">
                                ...
                                @forelse($classes as $c)
                                    <option value="{{$c->id}}" {{ old('class_id',$finalresult->class_id)==$c->id?"selected":""}}> {{ $c->class_name_en}}</option>
                                @empty
                                    <option value="">No Role found</option>
                                @endforelse
                                </select>
                            ...
                            <div class="col-12 col-lg-6">
                                ...
                                <input type="text" id="prac_marks" class="form-control" value="{{ old('total_marks',$finalresult->total_marks)}}" name="total_marks">
                                @if($errors->has('total_marks'))
                                    <span class="text-danger"> {{ $errors->first('total_marks') }}</span>
                                @endif
                            </div>
                            <div class="col-12 col-lg-6">
                                <label for="userName_en"><strong>Total GPA Marks</strong><i class="text-danger">*</i> </label>
                                <input type="text" id="total_gp" class="form-control" value="{{ old('total_gp',$finalresult->total_gp)}}" name="total_gp">
                            ...
                            <div class="col-12 col-lg-6">
                                ...
                                <input type="text" id="total" class="form-control" value="{{ old('total_gl',$finalresult->total_gl)}}" name="total_gl">
